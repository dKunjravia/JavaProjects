...
public class CurrentUserDataReader {
	private static CurrentUserData currentUsers;
	private static CurrentUserDataReader instance;
	
	private CurrentUserDataReader(CurrentUserData currentUsersData) {
		currentUsers = currentUsersData;
	}
	
	public static CurrentUserDataReader getInstance(CurrentUserData currentUsersData)
	{
		if(instance == null)
			instance= new CurrentUserDataReader(currentUsersData);
		return instance;
	}
	...
	public void parseUserData()
	{
		try {
			...
			Document currenUserdocument = documentBuilderObject.parse(new File("currentUsers.xml"));
			currenUserdocument.getDocumentElement().normalize();
			NodeList userList = currenUserdocument.getElementsByTagName("user");
